...
void HttpSession::on_read_header(const boost::system::error_code& ec, size_t bytes_transferred) {
    ...
    const std::string contentLengthStr(contentLength.data(), contentLength.size());
    m_result->size = atoi(contentLengthStr.c_str());
    auto location = msg["Location"];

    if (!location.empty()) {        
        const std::string locationStr(location.data(), location.size());
        auto session = std::make_shared<HttpSession>(m_service, locationStr, m_agent, m_timeout, m_isJson, m_result, m_callback);
        session->start();
        return close();
    }
